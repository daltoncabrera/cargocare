...
{
  public class FormRecepcion : KryptonForm
  {
    private const int TIPO_INSPECCION_RECEPCION = 2;
    ...
    private void prepareControls()
    {
      ...
      this.userControlGridSearchDespacho.SetEntity<inspeccionview>(this.db.inspeccionviews.Where<inspeccionview>((Expression<Func<inspeccionview, bool>>) (s => s.IdStatus != (int?) -1 && s.IdTipoInspeccion == (int?) 2)), (List<ColumnFilter>) null, (List<ColumnModel>) null, (List<ColumnModel>) null, (ColumnModel) null);
    ...
    {
      GUtils.ShowControl((Control) new UserControlRecepcion(selectedResult), "Editando Inspeccion", new Size?());
      this.userControlGridSearchDespacho.Refresh();
    }

    private void userControlGridSearchDespacho_OnAddEntity()
    {
      GUtils.ShowControl((Control) new UserControlRecepcion((List<SearchResult>) null), "Nueva Inspeccion", new Size?());
      this.userControlGridSearchDespacho.Refresh();
    }
